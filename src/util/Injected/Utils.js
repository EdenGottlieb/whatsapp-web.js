...
exports.LoadUtils = () => {
    ...
    window.WWebJS.pinUnpinMsgAction = async (msgId, action, duration) => {
        ...
        const originalFunction = window.safeRequire('WAWebPinMsgConstants').getPinExpiryDuration;
        window.safeRequire('WAWebPinMsgConstants').getPinExpiryDuration = () => duration;

        const response = await window.Store.PinnedMsgUtils.sendPinInChatMsg(message, action, duration);

        window.safeRequire('WAWebPinMsgConstants').getPinExpiryDuration = originalFunction;
